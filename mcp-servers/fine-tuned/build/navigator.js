 ...
 */
server.tool("agent-query", "Send a query to the Navigator API /api/v1/agent/query endpoint.", {
 ...
 */
server.tool("chat", "Initiate a chat session and fetch a response from the Chat API.", {
    applicationId: z.string().uuid().describe("<Required> Application UUID."),
    modelId: z.number().int().min(1).max(32767).describe("<Required> Model ID."),
    ...
    try {
        ...
        const { sessionId } = await initiateResponse.json();
 ...
 */
server.tool("rag-query", "Perform a RAG query and return the AI-generated answer.", {
